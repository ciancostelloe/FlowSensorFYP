...
def main():
    ...
    images, npaContours, npaHierarchy = cv2.findContours(imgThreshCopy,                 # input image, make sure to use a copy since the function will modify this image in the course of finding contours
    ...
    for npaContour in npaContours:                          # for each contour
        if cv2.contourArea(npaContour) > MIN_CONTOUR_AREA and cv2.contourArea(npaContour) < MAX_CONTOUR_AREA:          # if contour is big enough to consider
            ...
            if intChar == 27:                   # if esc key was pressed
                ...
                intClassifications.append(int(chr(intChar)))                                                # append classification char to integer list of chars (we will convert to float later before writing to file)
